...
class Seven_bar_mechanism(ap.Implicit_Problem):
    ...
    def f(self, t, y, yp):
        ...
        sibe, sith, siga, siph, side, siom, siep = sin(y[:7])
        cobe, coth, coga, coph, code, coom, coep = cos(y[:7])
        sibeth, cobeth = sin(beta + theta), cos(beta + theta)
        siphde, cophde = sin(phi + delta), cos(phi + delta)
        siomep, coomep = sin(omega + epsilon), cos(omega + epsilon)
        ...
        res_3 = zeros(6)  # Constraint vector (to be properly defined)
